...
function TrangleAnimation(Arr,Ymax) {
    /*根据数据 画出七个 三角形*/
    ...
    for (var i = 0; i < Arr.length; i++) {
        //判断方向
        var speedHei = EqualsOne((Arr[i].valuePx - OLD_DATA[i].n)/5);
        if(speedHei==0&&OLD_DATA[i].smallTitle!=2){
            OLD_DATA[i].smallTitle = 1;
        }

        if(speedHei==0&&OLD_DATA[i].smallTitle==2){
            HOW_MANY_I_DRAW.push(Arr[i]);
            OLD_DATA[i].height = Arr[i].valuePx;
            OLD_DATA[i].n = Arr[i].valuePx;
            OLD_DATA[i].value = Arr[i].value;
        }
        OLD_DATA[i].n +=speedHei;
...
function DrawTrangle(TrangleData, context) {
    ...
    context.strokeStyle = color.concat().replace(/,0\.\d+/, ',0.9');
    context.lineWidth = 2;
    context.beginPath();
    ...
    // 画圆
    context.beginPath();
    context.strokeStyle = color.concat().replace(/,0\.\d+/, '1');// 转译 颜色
...
function DrawTrangleTitle(TrangleData,context) {
         ...
         // 绘制 标题出场
          ...
          if(TrangleData.ns<20){
              ...
              if(TrangleData.ns2>=10){
                  ...
                  context.fillStyle="white";
                  context.fillText(TrangleData.name,x+110,500-height-10);
                  context.restore();
              ...
              context.save();
              context.fillStyle = TrangleData.color.concat().replace(/,0\.\d+/,',0.3');// 用正则来改变 颜色
              context.fillRect(x+100,500-height-30,TrangleData.ns2*10,TrangleData.ns2*3);
              context.restore();
    ...
    if(TrangleData.smallTitle==2){
        ...
        context.fillText(TrangleData.name,x+110,500-TrangleData.n-10);
        context.restore();

        context.save();
        context.fillStyle = TrangleData.color.concat().replace(/,0\.\d+/,',0.3');
        context.fillRect(x+100,500-TrangleData.n-30,TrangleData.ns2*10,TrangleData.ns2*3);
        context.restore();

        context.save();
        context.strokeStyle = "#8CDCFC";
        context.moveTo(x+50,500-height);
        context.lineTo(x+70,500-height-20);
        context.lineTo(x+100,500-height-20);
        context.stroke();
        context.restore();
...
function EqualsOne(a) {// 这里 使用了 chart1里面的误差判断
    if(a<0&&a>-1&&a<-0.1){
        a=-1;
    }
